package be.yorian.transactionAdapterBNP.parser;

import be.yorian.transactionAdapterBNP.mapper.TransactionMapper;
...
public class TextToTransactionParser {
    ...
    public List<TransactionDTO> filterTransactionsFromText() throws TextParserException {
        retrieveYear();
        Map<String, Integer> startlines = findTransactionsStarts();
        Map<String, String> txtexts = convertToTransactions(startlines);
        return createTransactions(txtexts).stream().map(TransactionMapper::mapTransactionToDTO).toList();
    }
